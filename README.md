...
- [ ] Fase 8 (raffinamenti) — IN CORSO, un pezzo alla volta:
  ...
  - [x] Slippage e commissioni nel backtest (`fase8_backtest.py`, tabella `backtest_risultati` in `database.py`) — simula la strategia SMA su ~2 anni di storico (usando la STESSA funzione `decidi_segnale()` della strategia vera, non una copia), applicando risk management, e lancia la simulazione due volte: una "ottimistica" (nessun costo) e una "realistica" (slippage 0.1% + commissione $1/ordine), mostrando quanto i costi ridurrebbero il risultato finale; non piazza nessun ordine vero — testato con dati sintetici (prezzo piatto = zero operazioni, ciclo su/giù con un BUY e una SELL verificati a mano, conferma che i costi riducono sempre il capitale finale e nella direzione giusta su ogni ordine)
  - [x] Analisi multi-timeframe (`fase8_multi_timeframe.py`, tabella `controlli_multi_timeframe` in `database.py`; nuova `scarica_prezzi_orari()` in `fase3_strategia_sma.py`; collegato in `fase4_esecuzione_ordini.py` come controllo aggiuntivo prima di ogni ordine BUY, dopo il filtro di liquidità e prima del position sizing) — oltre al segnale calcolato sulle candele giornaliere, guarda anche il trend delle candele ORARIE recenti: se il trend orario è chiaramente in discesa (contraddice il segnale giornaliero), il bot aspetta invece di comprare subito; non blocca mai per dati orari insufficienti, e non si applica mai al SELL (una posizione aperta deve poter sempre essere chiusa) — `decidi_segnale()` della strategia vera non è stata toccata, è solo un controllo in più, stessa logica dei filtri di liquidità/concentrazione già esistenti — testato con dati sintetici (trend in salita, in discesa, prezzo piatto, dati insufficienti, ripresa recente dopo un calo) e con un database temporaneo isolato per la nuova tabella
